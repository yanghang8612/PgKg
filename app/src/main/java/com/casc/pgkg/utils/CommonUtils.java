...
import com.casc.pgkg.bean.EPCType;
import com.casc.pgkg.bean.IntStrPair;
...
public class CommonUtils {
    ...
    private static String convertEPCHeader(byte[] epc) {
        String header = "";
        header += (char) epc[1];
        header += (char) epc[2];
        header += (char) epc[3];
    ...
    public static <T> List<T> fromJsonArray(Reply reply, Class<T[]> c) {
        T[] array = new Gson().fromJson(reply.getContent(), c);
